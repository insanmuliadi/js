    ...
    var j = function(a) {
      ...
      , g = function(z) {
        ...
        for (var q = 0; q < d.numPosts; q++) {
            ...
            r = ("media$thumbnail"in x[q] && d.thumbnailWidth !== false) ? x[q].media$thumbnail.url.replace(/(s\d+|w\d+(?:-[cp])?-h\d+)(?:-[cp])?(?:-rw)??/, "/w" + d.thumbnailWidth + "-h" + d.thumbnailHeight + "-p-rw") : d.noImage;
			rbk = ("media$thumbnail"in x[q] && d.thumbnailWidth !== false) ? x[q].media$thumbnail.url.replace(/(s\d+|w\d+(?:-[cp])?-h\d+)(?:-[cp])?(?:-rw)??/, "/w48-h48-p-rw") : d.noImage;
        ...
        s.innerHTML = c += "</ul>" + y;
        d.callBack();
        const newBookmarkButtons = document.querySelectorAll('.bookmark-button');
        if (config.showBookmarks === true){
          newBookmarkButtons.forEach(function(element) {
